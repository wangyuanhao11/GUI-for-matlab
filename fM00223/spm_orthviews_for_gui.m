function varargout = spm_orthviews_for_gui(action,varargin)
    ...
    case 'caption'
        if ~isnumeric(varargin{1})
            ...
            for i=1:numel(varargin{1})
                h = spm_orthviews_for_gui('Caption',i,varargin{1}{i},varargin{3:end});
...
H = ancestor(st.vols{vh}.ax{1}.ax,{'figure','uitab'});
...
if ~strcmpi(get(gcbf,'SelectionType'),'alt')
    ...
    spm_orthviews_for_gui('reposition');
end
...
centre = spm_orthviews_for_gui('Pos');
spm_orthviews_for_gui('setcoords',centre+[0 0 ScrollWheelData.VerticalScrollCount]');
...
global ss;
            cmap = get(ss.UIFigure,'Colormap');
%             ss.UIFigure.Colormap=gray(256);
            if size(cmap,1)~=128
                
                figure(ss.UIFigure)
               ss.UIFigure.Colormap= spm_colourmap('gray-hot')
...
function centre = findcent
global st;global ss;
obj    = get(ss.UIFigure,'CurrentObject');
centre = [];
cent   = [];
cp     = [];
for i=valid_handles
...
spm_orthviews_for_gui('context_menu','image_info',item00,volhandle);
...
[zl, rl] = spm_orthviews_for_gui('ZoomMenu');
for cz = numel(zl):-1:1
    if isinf(zl(cz))
        czlabel = 'Full Volume';
    elseif isnan(zl(cz))
...
switch lower(varargin{1})
    ...
    case 'xhair'
        spm_orthviews_for_gui('Xhairs',varargin{2:end});
    ...
    case 'orientation'
        ...
        if varargin{2} == 3
            spm_orthviews_for_gui('Space');
            for i = 1:numel(cm_handles),
                z_handle = findobj(cm_handles(i),'label','World space');
                set(z_handle,'Checked','on');
            end
        elseif varargin{2} == 2,
            spm_orthviews_for_gui('Space',1);
            for i = 1:numel(cm_handles)
            ...
            spm_orthviews_for_gui('Space',get_current_handle);
    ...
    case 'window'
        ...
        elseif varargin{2} == 3
            ...
            spm_orthviews_for_gui('window',current_handle,w);
        end
    ...
    case 'swap_img'
        ...
        if ~isempty(newimg)
            ...
            spm_orthviews_for_gui('context_menu','image_info',get(gcbo, 'parent'));
            redraw_all;
        end
...
for i=valid_handles(handles)
    if isfield(st.vols{i}.ax{1},'cm')
        set(findobj(st.vols{i}.ax{1}.cm,'UserData','pos_mm'),...
            'Label',sprintf('mm:  %.1f %.1f %.1f',spm_orthviews_for_gui('pos')));
        pos = spm_orthviews_for_gui('pos',i);
